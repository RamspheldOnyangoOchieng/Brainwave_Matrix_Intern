...
from models.user import UserCreate, User, User as UserModel
...
def login():
    ...
    try:
        user_id = asyncio.run(verify_credentials(data.email, data.password))
        if not user_id:
...
def request_password_reset():
    ...
    try:
        reset_token = asyncio.run(generate_reset_token(email))
        if not reset_token:
...
def reset_password_endpoint():
    ...
    try:
        success = asyncio.run(reset_password(token, new_password))
        if not success:
...
def users():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Insert into DB
            user_record = asyncio.run(db.insert_record('users', user_dict))
        ...
        try:
            users = asyncio.run(db.execute_query("SELECT * FROM users"))
...
def user_operations(user_id):
    try:
        ...
        if request.method == 'GET':
            user_record = asyncio.run(db.get_record('users', {'id': user_uuid}))
            if not user_record:
                return jsonify({"error": "User not found."}), 404
            user_record.pop('password', None)
            ...
            updated_user = asyncio.run(db.update_record('users', {'id': user_uuid}, data))
            if not updated_user:
                return jsonify({"error": "User not found."}), 404
            ...
            success = asyncio.run(db.delete_record('users', {'id': user_uuid}))
            if not success:
                return jsonify({"error": "User not found."}), 404
...
def list_accounts():
    try:
        ...
        accounts = asyncio.run(db.execute_query(query))
...
def check_balance(account_id):
    try:
        ...
        result = asyncio.run(atm_service.check_balance(account_uuid))
...
def deposit(account_id):
    try:
        account_uuid = handle_uuid(account_id)
        data = request.validated_data
        transaction = asyncio.run(atm_service.deposit(account_uuid, data.amount))
...
def withdraw(account_id):
    try:
        account_uuid = handle_uuid(account_id)
        data = request.validated_data
        transaction = asyncio.run(atm_service.withdraw(account_uuid, data.amount))
        return jsonify(transaction.dict())
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
    except Exception as e:
...
def transfer():
    try:
        ...
        result = asyncio.run(atm_service.transfer(from_uuid, to_uuid, data.amount))
        return jsonify({
...
def account_history(account_id):
    try:
        ...
        transactions = asyncio.run(db.execute_query(query, query_params))
